...
void print_tree(std::vector<BufferControlBlock *>bcb_list) {
	for(unsigned i = 0; i < bcb_list.size(); i++) {
...
void BufferTree::setup_tree() {
	...
	for (unsigned l = 1; l <= max_level; l++) { // loop through all levels
		unsigned level_size  = pow(B, l); // number of blocks in this level
		unsigned plevel_size = pow(B, l-1);
		unsigned start       = buffers.size();
		Node key             = 0;
		...
		for (unsigned i = 0; i < level_size; i++) { // loop through all blocks in the level
...
inline uint32_t which_child(Node key, Node min_key, Node max_key, uint8_t options) {
	...
	Node larger_kids  = total % options;
	Node larger_count = larger_kids * ceil(div);
...
flush_ret_t BufferTree::do_flush(char *data, uint32_t data_size, uint32_t begin, 
	...
	while (data - data_start < data_size) {
		...
		if (flush_pos[child] - flush_buf[child] >= full_flush) {
			...
			File_Pointer size = flush_pos[child] - flush_buf[child];
	...
	for (uint8_t i = 0; i < B; i++) {
		if (flush_pos[i] - flush_buf[i] > 0) {
			...
			File_Pointer size = flush_pos[i] - flush_buf[i];
